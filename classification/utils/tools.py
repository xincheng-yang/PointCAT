...
def sample_and_group(npoint, radius, nsample, xyz, points):
    ...
    new_points = grouped_points_norm
...
def save_model(net, epoch, path, acc, is_best, **kwargs):
    ...
    #  last_parameters = net.state_dict()
    for key, value in kwargs.items():
        state[key] = value
    filepath = os.path.join(path, "last_checkpoint.pth")
    #  parameterspath = os.path.join(path, "last_parameters.pth")
    torch.save(state, filepath)
    #  torch.save(last_parameters, parameterspath)
    if is_best:
        shutil.copyfile(filepath, os.path.join(path, 'best_checkpoint.pth'))
        #  shutil.copyfile(parameterspath, os.path.join(path, 'best_parameters.pth'))
