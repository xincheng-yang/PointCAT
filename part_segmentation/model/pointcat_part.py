...
class PointCAT_part(nn.Module):
    # noinspection PyDefaultArgument
    def __init__(self, class_num=50, channel_list=[64, 128, 256, 512], sample_list=[512, 128, 32, 8],
                 de_inchannel_list=[1536, 768, 384, 192], de_outchannel_list=[512, 256, 128, 128], k_neighbors=32,
                 small_dim=1024, large_dim=512):
        super(PointCAT_part, self).__init__()
        ...
        self.cls_emb = nn.Sequential(
            nn.Conv1d(in_channels=16, out_channels=32, kernel_size=1, bias=True),
            nn.BatchNorm1d(32),
            nn.LeakyReLU(negative_slope=0.01, inplace=True),
        ...
        self.classifier = nn.Sequential(
            nn.Conv1d(320, 128, 1, bias=True),
            nn.BatchNorm1d(128),
            nn.Dropout(),
            nn.Conv1d(128, class_num, 1, bias=True)
        )

    def forward(self, x, norm, cls_label):
        ...
        cls_token = self.cls_emb(cls_label.unsqueeze(dim=-1))
        x = x.permute(0, 2, 1)
        x = torch.cat([x, head_token.repeat([1, 1, x.shape[-1]]), global_feature.repeat([1, 1, x.shape[-1]]), cls_token.repeat([1, 1, x.shape[-1]])], dim=1)
        x = self.classifier(x)
...
class Vertax(nn.Module):
    # noinspection PyDefaultArgument
    def __init__(self, group_list=[256, 256, 256, 256], feature_nums=[256, 256, 256, 256],
                 multihead_layer=[4, 4, 4, 4], class_num=40):
        super(Vertax, self).__init__()
        self.levels = len(group_list)
        self.class_num = class_num
        self.SampleAndGroup_list = nn.ModuleList()
        self.MultiHeadAtt_list = nn.ModuleList()
        self.classifier = Cls(in_channel=1024)
        for i in range(self.levels):
            group_index = group_list[i]
            Multi_index = multihead_layer[i]
            feature_index = feature_nums[i]
            self.SampleAndGroup_list.append(SampleAndGroup(num_points=group_index, feature_channel=feature_index))
    ...
    def forward(self, x):
        ...
        feature_list = []
        for i in range(self.levels):
            x = self.SampleAndGroup_list[i](xyz)  # 32, n, 64 // B, N, C
            _, N, _ = x.shape
            feature = self.MultiHeadAtt_list[i](x)
            feature = F.adaptive_max_pool1d(feature.permute(0, 2, 1), 1).view(B, -1)  # B, N
