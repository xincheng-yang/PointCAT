...
def train(args, io):
    ...
    if args.resume:
        ...
        print(torch.load("checkpoints/%s/best_insiou_model.pth" %
              args.exp_name).keys())
    else:
        print("Training from scratch...")

    # =========== Dataloader =================
    train_data = PartNormalDataset(
        npoints=2048, split='trainval', normalize=False)
    ...
    if args.use_sgd:
        ...
        print("Use Adam")
        opt = optim.Adam(model.parameters(), lr=args.lr, betas=(
            0.9, 0.999), eps=1e-08, weight_decay=0)

    if args.scheduler == 'cos':
        print("Use CosLR")
        scheduler = CosineAnnealingLR(
            opt, args.epochs, eta_min=args.lr if args.use_sgd else args.lr / 100)
    else:
        print("Use StepLR")
    ...
    for epoch in range(args.epochs):
        ...
        train_epoch(train_loader, model, opt, scheduler,
                    epoch, num_part, num_classes, io)

        test_metrics, total_per_cat_iou = test_epoch(
            test_loader, model, epoch, num_part, num_classes, io)
        ...
        if test_metrics['accuracy'] > best_acc:
            ...
            torch.save(state, 'checkpoints/%s/best_acc_model.pth' %
                       args.exp_name)
        ...
        if test_metrics['shape_avg_iou'] > best_instance_iou:
            ...
            torch.save(state, 'checkpoints/%s/best_insiou_model.pth' %
                       args.exp_name)
        ...
        if avg_class_iou > best_class_iou:
            ...
            torch.save(state, 'checkpoints/%s/best_clsiou_model.pth' %
                       args.exp_name)
    ...
    torch.save(state, 'checkpoints/%s/model_ep%d.pth' %
...
def train_epoch(train_loader, model, opt, scheduler, epoch, num_part, num_classes, io):
    ...
    for batch_id, (points, label, target, norm_plt) in tqdm(enumerate(train_loader), total=len(train_loader), smoothing=0.9):
        ...
        # target: b,n
        seg_pred = model(points, norm_plt, to_categorical(
            label, num_classes))  # seg_pred: b,n,50
        loss = F.nll_loss(seg_pred.contiguous().view(-1,
                          num_part), target.view(-1, 1)[:, 0])

        # instance iou without considering the class average at each batch_size:
        # list of of current batch_iou:[iou1,iou2,...,iou#b_size]
        batch_shapeious = compute_overall_iou(seg_pred, target, num_part)
        # total iou of current batch in each process:
        batch_shapeious = seg_pred.new_tensor(
            [np.sum(batch_shapeious)], dtype=torch.float64)  # same device with seg_pred!!!
        ...
        # torch.int64: total number of correct-predict pts
...
def test_epoch(test_loader, model, epoch, num_part, num_classes, io):
    ...
    for batch_id, (points, label, target, norm_plt) in tqdm(enumerate(test_loader), total=len(test_loader), smoothing=0.9):
        ...
        for shape_idx in range(seg_pred.size(0)):  # sample_idx
            ...
            # count the number of this cat is chosen
            final_total_per_cat_seen[cur_gt_label] += 1

        # total iou of current batch in each process:
        batch_ious = seg_pred.new_tensor(
            [np.sum(batch_shapeious)], dtype=torch.float64)  # same device with seg_pred!!!
    ...
    for cat_idx in range(16):
        ...
        if final_total_per_cat_seen[cat_idx] > 0:
            final_total_per_cat_iou[cat_idx] = final_total_per_cat_iou[cat_idx] / \
                final_total_per_cat_seen[cat_idx]  # avg class iou across all samples
...
def test(args, io):
    ...
    for batch_id, (points, label, target, norm_plt) in tqdm(enumerate(test_loader), total=len(test_loader), smoothing=0.9):
        batch_size, num_point, _ = points.size()
        points, label, target, norm_plt = Variable(points.float()), Variable(
            label.long()), Variable(target.long()), Variable(norm_plt.float())
        points = points.transpose(2, 1)
        norm_plt = norm_plt.transpose(2, 1)
    ...
    for cat_idx in range(16):
        if total_per_cat_seen[cat_idx] > 0:
            total_per_cat_iou[cat_idx] = total_per_cat_iou[cat_idx] / \
                total_per_cat_seen[cat_idx]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', type=str, default='PointCAT_part')
    parser.add_argument('--exp_name', type=str, default='default', metavar='N',
    ...
    if not args.eval:
        io = IOStream('checkpoints/' + args.exp_name +
                      '/%s_train.log' % (args.exp_name))
    else:
        io = IOStream('checkpoints/' + args.exp_name +
                      '/%s_test.log' % (args.exp_name))
    io.cprint(str(args))
