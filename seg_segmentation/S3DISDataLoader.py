...
class S3DISDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.transform is not None:
            current_points[:, :3], current_points[:, 3:6] = self.transform(current_points[:, :3], current_points[:, 3:6])
            # current_xyz = current_points[:, :3]
            # current_colors = current_points[:, 3:6]
            # current_xyz, current_colors = self.transform(current_xyz, current_colors)
            # current_points = np.concatenate((current_xyz, current_colors), axis=-1)
