...
class PolyLRwithWarmup(LambdaStepLR):
    ...
    def __init__(self, optimizer, max_iter, warmup='linear', warmup_iters=1500, warmup_ratio=1e-6, power=1.0,
                 last_step=-1):
...
class MultiStepWithWarmup(LambdaStepLR):
    def __init__(self, optimizer, milestones, gamma=0.1, warmup='linear', warmup_iters=1500, warmup_ratio=1e-6,
                 last_step=-1):
...
def initialize_scheduler(optimizer, config, last_epoch=-1, scheduler_epoch=True, logger=None):
    ...
    if config.scheduler == 'StepLR':
        ...
        return PolyLRwithWarmup(optimizer, max_iter=config.max_iter, warmup=config.warmup,
                                warmup_iters=config.warmup_iters, warmup_ratio=config.warmup_ratio,
                                power=config.poly_power, last_step=last_step)
        ...
        return OneCycleLR(optimizer, max_lr=config.oc_max_lr, total_steps=config.max_iter,
...
if __name__ == '__main__':
    ...
    scheduler = OneCycleLR(optimizer, max_lr=0.2, total_steps=max_iter, pct_start=0.1, anneal_strategy='cos',
                           div_factor=25.0,
